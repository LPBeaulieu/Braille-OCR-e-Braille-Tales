...
#(PEF) file and transposition to printed English (RTF document) will be performed.
    ...
    with alive_bar(len(JPEG_file_names)) as bar:
        ...
        with open(path + OCR_text_file_name + '-OCR results.txt', 'a+') as f:
            for i in range(len(JPEG_file_names)):
                ...
                #remain after the whille loop.
                for i in range((int(len(character_list)/41))-1, -1, -1):
                    current_index = i*41-2
                    if character_list[current_index] == "⠀" and character_list[current_index+1] == "⠀":
                ...
                #Instances of two or more empty braille cells are changed for
                #single empty braille cells for clarity.
...
#(PEF) file and transposition to printed English (RTF document) will be performed.
    ...
    with open(cwd + "/OCR Raw Data/" + file_name, "r") as g:
        ...
        #Instances of two or more empty braille cells are changed for
        #single empty braille cells for clarity (just in case excessive
        #empty braille cells were introduced durring corrections).
        character_string = re.sub('["⠀"]{2,}', "⠀", character_string)
...
#needed in the English print transposed form. However, these will remain in the
...
#transposition Python code below looks at the character following a match in order to decide
#on the transposition outcome, and it wouldn't make sense to add specific "else" statements
...
#to a letter or to letters. Because of this ambiguity, the transposition of the final-letter
...
"⠖":"!", "⠄":"’", "⠤":"-", "⠦":'“', "⠴":'”', "⠒": ":",
"⠆": ";", "⠶": r"\'27", "⠔":"⠰⠔", "⠢":"⠰⠢"}
#When the grade I passage indicator "⠰⠰⠰" is encountered, grade I transposition
...
for i in range(len(grade_I_passage_match_indices)-1, -1, -1):
    ...
    #transposed passage is then added and the remainder of "new_character_string" starting three
    ...
    try:
        ...
        new_character_string = (new_character_string[:grade_I_passage_match_indices[i]] +
        "[Transposition note: a grade I passage indicator was located here, but no grade I terminator was found after it.]⠀" +
        new_character_string[grade_I_passage_match_indices[i]+3:])

#When the grade I word indicator "⠰⠰" is encountered, grade I transposition continues
...
for i in range(len(grade_I_word_match_indices)-1, -1, -1):
    ...
    #to "⠰⠰" (skipping over the grade I initiator). The grade I transposed
    ...
    #transposed using grade I braille.
...
for i in range(len(grade_I_symbol_match_indices)-1, -1, -1):
    ...
    if match_found == False and grade_I_symbol_match_indices[i] == len(new_character_string) -2:
        try:
            ...
            new_character_string = (new_character_string[:grade_I_symbol_match_indices[i]] +
            "[Transposition note: a grade I symbol character was found here, but the following character was not recognized as a letter, and so could not be transposed in grade I.]⠀" +
...
#The following section deals with numerals, which are transposed on a one-to-one basis
...
"⠋":"6", "⠛":"7", "⠓":"8", "⠊":"9", "⠚":"0", "⠂": ",", "⠲": ".", "⡈":"/"}
#When the numeric indicator "⠼" is encountered, transposition of the numerals continue as long as
...
for i in range(len(numeric_symbol_match_indices)-1, -1, -1):
    ...
    #the numeric symbol in the document) the transposition to numbers occurs until the
...
#symbols must be processed before individually transposing the remaining characters on a one to one basis
...
#All following words need to stand alone in order to be transposed to their printed English form.
...
#that the braille equivalent of "‘Was’," ("⠠⠦⠠⠴⠠⠴") needs to be transposed to printed English,
...
for i in range(len(capitalized_his_match_indices)-1, -1, -1):
    ...
    #as a possibility following a "⠠⠦" match for it to be transposed to "His", as
...
#be transposed to the apostrophe.
...
#double quote ('⠠⠶', '"') could be transposed by the code to the capitalized
...
#be transposed at the very end of the code and not covered in this section).
...
for lower_wordsign in lower_wordsigns:
    ...
    for i in range(len(lower_wordsign_match_indices)-1, -1, -1):
        ...
        #leaves the opening double quote "“", which will be transposed later in the code.
...
#by the first character of the different typeform indicators will be transposed to "be".
...
for i in range(len(be_match_indices)-1, -1, -1):
    ...
    #typeform indicator and can be transposed to "be".
...
for i in range(len(capitalization_passage_match_indices)-1, -1, -1):
    ...
    #proofreading their text.
        ...
        new_character_string = (new_character_string[:capitalization_passage_match_indices[i]] +
        "[Transposition note: a capitalization passage indicator was located here, but no capitalization terminator was found after it.] " +
...
for i in range(len(italics_passage_match_indices)-1, -1, -1):
    ...
    #the user in proofreading their text.
        ...
        new_character_string = (new_character_string[:italics_passage_match_indices[i]] +
        "[Transposition note: an italics passage indicator was located here, but no italics terminator was found after it.] " +
...
for i in range(len(bold_passage_match_indices)-1, -1, -1):
    ...
    #proofreading their text.
        ...
        new_character_string = (new_character_string[:bold_passage_match_indices[i]] +
        "[Transposition note: a bold passage indicator was located here, but no bold terminator was found after it.] " +
...
for i in range(len(underline_passage_match_indices)-1, -1, -1):
    ...
    #the user in proofreading their text.
        ...
        new_character_string = (new_character_string[:underline_passage_match_indices[i]] +
        "[Transposition note: An underline passage indicator was located here, but no underline terminator was found after it.] " +
...
for i in range(len(script_passage_match_indices)-1, -1, -1):
    ...
    #the user in proofreading their text.
        ...
        new_character_string = (new_character_string[:script_passage_match_indices[i]] +
        "[Transposition note: a script passage indicator was located here, but no script terminator was found after it.] " +
...
#constituting an RTF escape). Now that braille transposition is complete, they must
...
#The instances of two or more successive empty braille cells are changed for a
#single empty braille cell. However, two successive empty braille cells must remain
#for there to be an optional space after an RTF command followed by an actual space,
#which would only translate to a single space in the RTF document. An empty braille
#cell is then added after every closing RTF command for bold ("\b0"), italics ("\i0"),
#caps ("\caps0"), smallcaps ("\scaps0"), strikethrough ("\strike0") and underline ("\ul0")
#in order to accomodate for the optional space after RTF commands. This will result in an
#actual space being introduced after an RTF command. If users wish to avoid this, they
#could opt for placing only the opening RTF command in curly brackets:
#(ex: "{\b your text here}other text").
...
new_character_string = (re.sub('["⠀"]{2,}', "⠀", new_character_string).replace(r"\b0", r"\b0⠀")
.replace(r"\i0", r"\i0⠀").replace(r"\caps0", r"\caps0⠀").replace(r"\scaps0", r"\scaps0⠀")
.replace(r"\strike0", r"\strike0⠀").replace(r"\ul0", r"\ul0⠀").replace("⠀)", ")")
.replace("⠀?", "?").replace("⠀!", "!").replace("⠀,", ",").replace("⠀:", ":")
.replace("⠀;", ";").replace("⠀\\'94", "\\'94").replace("⠀\\'92", "\\'92"))
...
#had been added at the end of the OCR document because some of the transposition
#Python code looks at the character following a match in order to decide on the
#transposition outcome, and it wouldn't make sense to add specific "else" statements
#to account for all these case scenarios, as the words wouldn't normally be found at the very
#end of the document in the first place, but would rather be followed by a punctuation mark.
