...
#empty braille cells ("</row>\n\t\t\t\t\t<row>⠀⠀"). The addition of empty braille
#cells must take place after instances of two or more empty braille cells were
#converted into single empty braille cells in the code above. An empty
#braille cell is included at the end of every pattern in the lines below to avoid
#being left over with excessive spaces, as PEF tags don't allow for optional
#spaces the way RTF tags do, and since the braille will not be submitted to
#transposition code that requires determining whether the braille characters that
#follow the patterns are free standing or not.
